...
public class RSFontSystem extends DrawingArea {
				...
				if (character == 60) {
					...
					if (character == 62 && startIndex != -1) {
						...
						if (effectString.equals(startEffect)) {
							...
							if (effectString.startsWith(startUnderline)) {
								try {
							...
							if (transparency == 256) {
								if (textShadowColor != -1) {
									drawCharacter(character, drawX + characterDrawXOffsets[character] + 1,
											drawY + characterDrawYOffsets[character] + 1, width, height,
								...
								drawTransparentCharacter(character, drawX + characterDrawXOffsets[character],
										drawY + characterDrawYOffsets[character], width, height, textColor,
										transparency, false);
							}
						} else if (anInt4178 > 0) {
							anInt4175 += anInt4178;
					...
					if (effectString.equals(startEffect)) {
						...
						if (effectString.startsWith(startImage)) {
							try {
								...
								Sprite icon = imageId == 997 ? Client.instance.eventManager : imageId == 998 ? Client.instance.communityManager : imageId == 999 ? Client.instance.groupIron : SpriteLoader.sprites[679 + imageId];
				...
				if (character == 62 && startIndex != -1) {
					...
					if (effectString.equals(startEffect)) {
						...
						if (effectString.startsWith(startImage)) {
							try {// <img=
								int iconId = Integer.valueOf(effectString.substring(4));
								finalWidth += iconId == 997 ? Client.instance.eventManager.myWidth : iconId == 998 ? Client.instance.communityManager.myWidth : iconId == 999 ? Client.instance.groupIron.myWidth : SpriteLoader.sprites[679 + iconId].myWidth;
							} catch (Exception exception) {
								/* empty */
							}
						}
	...
	public static String handleOldSyntax(String text) {
		...
		text = replace(text, "@dbl@", "<col=00000>");
		text = replace(text, "@dgr@", "<col=d47519>");
		text = replace(text, "@enc@", "<col=e39be9>");
		text = replace(text, "@tox@", "<col=2c700f>");
		text = replace(text, "@gre@", "<col=65280>");
		text = replace(text, "@blu@", "<col=001a80>");
		...
		text = replace(text, "@cr18@", "<img=16>");
		text = replace(text, "@cr2@", "<img=2>");
		text = replace(text, "@cr3@", "<img=3>");
		text = replace(text, "@cr19@", "<img=17>");
		text = replace(text, "@cr20@", "<img=18>");
		text = replace(text, "@dev@", "<img=3>");
		text = replace(text, "@des@", "<img=4>");
		text = replace(text, "@vet@", "<img=5>");
		text = replace(text, "@don@", "<img=6>");
		text = replace(text, "@osh@", "<col=64431c>");
		text = replace(text, "@or2@", "<col=ff7000>");
		text = replace(text, "@dom@", "<col=3a8475>");
		text = replace(text, "@pur@", "<col=71168e>");
