...
public class RSApplet extends Applet implements Runnable, MouseListener, MouseMotionListener, MouseWheelListener,
	...
	public void mouseWheelMoved(MouseWheelEvent event) {
		...
		} else if (mouseX > 0 && mouseX < 512 && mouseY > Client.getClient().clientHeight - 165
	...
	public boolean handleInterfaceScrolling(MouseWheelEvent event) {
		...
		if(Client.backDialogID == 33400 && mouseX >= 0 && mouseX <= 516 && mouseY >= 338 && mouseY <= 470) {
			RSInterface widget = RSInterface.interfaceCache[33404];
			widget.scrollPosition += rotation * 30;
			if(widget.scrollPosition < 0)
				widget.scrollPosition = 0;
			else if(widget.scrollPosition > widget.scrollMax)
				widget.scrollPosition = widget.scrollMax;
			return true;
		}
		return false;
	}
	...
	public final void mousePressed(MouseEvent e) {
        ...
        Client.instance.mousePressed = true;
	...
	public final void mouseReleased(MouseEvent e) {
		int x = e.getX();
		int y = e.getY();
        Client.instance.mousePressed = false;
		if (mainFrame != null) {
			Insets insets = mainFrame.getInsets();
			x -= insets.left;// 4
			y -= insets.top;// 22
        ...
        if(Client.instance.loggedIn) {
            RSInterface viewDistanceInterface = RSInterface.interfaceCache[26286];
            int viewDistance = (int) viewDistanceInterface.widgetSlider.getValue();
            if (viewDistance >= 25 && viewDistance != ClientSettings.viewDistance) {
                ClientSettings.viewDistance = viewDistance;
	...
	public final void keyPressed(KeyEvent keyevent) {
		...
		if(i == KeyEvent.VK_ESCAPE) {
			...
			if(Client.instance.variousSettings[363] == 1) {
				Client.instance.bankSearch = "";
				Client.instance.variousSettings[363] = 0;
				Client.instance.stoppedSearching = true;
				Client.instance.showInput = false;
				Client.instance.backDialogID = -1;
			}
	...
	private static final Color LOADING_COLOR = new Color(240, 174, 27);
    ...
    void drawLoadingText(int i, String s, boolean center) {
        while(graphics == null) {
            graphics = getGameComponent().getGraphics();
            try {
                getGameComponent().repaint();
        ...
        Color color = new Color(	240, 174, 27); //140, 17, 17
        int drawY = height / 2 - 18;
        graphics.setColor(color);
        graphics.drawRect(width / 2 - 152, drawY, 304, 33);
        graphics.fillRect(width / 2 - 150, drawY + 2, i * 3, 30);
        graphics.setColor(Color.black);
        graphics.fillRect((width / 2 - 150) + i * 3, drawY + 2, 300 - i * 3, 30);
        graphics.setFont(font);
        graphics.setColor(Color.gray);
        graphics.drawString(s, (width - fontmetrics.stringWidth(s)) / 2, drawY + 22);
    }
