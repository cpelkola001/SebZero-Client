...
//		System.out.println("Skulled: " + skulled);
		...
		desc = null;
		team = 0;
		for (int partId = 0; partId < equipment.length; partId++) {
			...
			// System.out.println("equipment["+partId+"]: " + equipment[partId] + ",
			// equipment[partId] - 512: " + (equipment[partId] - 512));
...
//		System.out.println("Rights: " + playerRights);
		...
		for (int k1 = 0; k1 < equipment.length; k1++) {
			aLong1718 <<= 4;
			if (equipment[k1] >= 256)
...
//		System.out.println("Arrays player equipment: " + Arrays.toString(this.equipment));
	...
	public Model method452() {
		...
		if (model_1 == null) {
			...
			for (int bodyPartId = 0; bodyPartId < equipment.length; bodyPartId++) {
		...
		if (model_1 == null) {
			...
			for (int currentPart = 0; currentPart < 13; currentPart++) {
				...
				if (i3 >= 512) {
					 ...
					 */
					if (currentPart < modifiedColors.length && modifiedColors[currentPart] != null) {
